...
from .render import (KIND_ASSISTANT, KIND_ERROR, KIND_INFO, KIND_SPACER, KIND_TOOL,
                     KIND_USER, Block, activity, header, status_bar)
...
class App:
    ...
    def banner(self) -> None:
        ...
        self.add(KIND_INFO, f"папка {ws.root}")
        self.add(KIND_INFO,
                 f"модель {model} ×{mult:g} · режим правок «{self.agent.confirm_mode}» · "
                 f"шагов на задачу {self.agent.max_steps} · кэш промпта "
                 f"{'вкл' if self.gw.cache_enabled else 'выкл'}")
        ...
        self.add(KIND_INFO, "задачу пишите словами · /help — команды · /exit — выход")
    ...
    def maybe_ask_workspace(self) -> None:
        ...
        if self.ws.root == project_root():
            self.add(KIND_INFO, "консоль открыта в папке самой программы — выберите, "
                                "где лежит ваш код")
            self.cmd_cd("")
        else:
    ...
    def cmd_cd(self, arg: str) -> None:
        ...
        self.open_dialog(ListDialog("Рабочая папка", items, allow_text=True),

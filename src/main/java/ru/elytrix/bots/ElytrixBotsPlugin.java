...
public final class ElytrixBotsPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void reconcileVisible(){int wanted=visibleTarget(),current=0;for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving!=null)current++;while(current>wanted){for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving!=null){liveBots.remove(bot.moving);bot.moving.player.tick(Collections.emptySet());bot.moving=null;registry.remove(bot.player.getUuid());current--;break;}}if(current<wanted){for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving==null){World world=Bukkit.getWorld(getConfig().getString("population.region.world","spawn"));if(world==null)continue;Point point=randomSafePoint(world);registry.register(bot.profile.name,bot.player.getUuid(),world);Location body=new Location(world,bot.player.getPos().x,bot.player.getPos().y,bot.player.getPos().z);registry.relocate(bot.player.getUuid(),body);bot.moving=new MovingBot(bot.player,world,point.vector(),3.4+random.nextDouble());liveBots.add(bot.moving);for(Player viewer:realPlayers())registry.hideTab(bot.player.getUuid(),viewer);if(++current>=wanted)break;}}}

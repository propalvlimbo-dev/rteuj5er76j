...
public final class ElytrixBotsPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void reconcileVisible(){int wanted=visibleTarget(),current=0;for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving!=null)current++;while(current>wanted){for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving!=null){World world=bot.moving.world;liveBots.remove(bot.moving);bot.moving.player.tick(Collections.emptySet());bot.moving=null;registry.relocate(bot.player.getUuid(),new Location(world,1000.5,100,1000.5));Player entity=registry.player(bot.player.getUuid());if(entity!=null)entity.setMetadata("vanished",new FixedMetadataValue(this,true));current--;break;}}if(current<wanted){for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving==null){World world=Bukkit.getWorld(getConfig().getString("population.region.world","spawn"));if(world==null)continue;Point point=randomSafePoint(world);Location body=new Location(world,bot.player.getPos().x,bot.player.getPos().y,bot.player.getPos().z);registry.relocate(bot.player.getUuid(),body);Player entity=registry.player(bot.player.getUuid());if(entity!=null)entity.setMetadata("vanished",new FixedMetadataValue(this,false));bot.moving=new MovingBot(bot.player,world,point.vector(),3.4+random.nextDouble());liveBots.add(bot.moving);if(++current>=wanted)break;}}}
    ...
    private boolean activateOne(boolean manual,boolean visible){
        ...
        BotProfile profile=available.get(random.nextInt(available.size()));pendingProfiles.add(profile.name);World botWorld=Bukkit.getWorld(getConfig().getString("population.region.world","spawn"));Location spawn;if(botWorld!=null){Point start=randomSafePoint(botWorld);spawn=new Location(botWorld,start.x(),start.y(),start.z(),start.yaw(),start.pitch());}else spawn=spawn(null);VirtualPlayer player=create(profile.name,profile.ping,profile.group,spawn.getWorld());
    ...
    private void startFanConversation(){
        Bukkit.getScheduler().runTaskLater(this,()->{
            List<ActiveBot> list=new ArrayList<>();for(ActiveBot bot:active.values())if(registry.player(bot.player.getUuid())!=null)list.add(bot);if(list.isEmpty())return;chat(list.get(random.nextInt(list.size())),"Кто смотрит rooyzee?");
...
List<ActiveBot> list=new ArrayList<>();for(ActiveBot bot:active.values())if(registry.player(bot.player.getUuid())!=null)list.add(bot);if(list.isEmpty())return;

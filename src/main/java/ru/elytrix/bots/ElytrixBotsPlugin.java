...
public final class ElytrixBotsPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void reconcileVisible(){int wanted=visibleTarget(),current=0;for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving!=null)current++;while(current>wanted){for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving!=null){World world=bot.moving.world;liveBots.remove(bot.moving);bot.moving.player.tick(Collections.emptySet());bot.moving=null;registry.relocate(bot.player.getUuid(),new Location(world,1000.5,100,1000.5));current--;break;}}if(current<wanted){for(ActiveBot bot:active.values())if(!bot.manual&&bot.moving==null){Player entity=registry.player(bot.player.getUuid());if(entity==null)continue;Point point=randomSafePoint(entity.getWorld());Location body=new Location(point.world(),bot.player.getPos().x,bot.player.getPos().y,bot.player.getPos().z);registry.relocate(bot.player.getUuid(),body);bot.moving=new MovingBot(bot.player,point.world(),point.vector(),3.4+random.nextDouble());liveBots.add(bot.moving);if(++current>=wanted)break;}}}

...
final class NmsFakePlayerRegistry {
    ...
    void relocate(UUID uuid,org.bukkit.Location location){org.bukkit.entity.Player player=player(uuid);if(player==null)return;try{player.teleport(location);Object entity=byUuid.get(uuid);entity.getClass().getMethod("setInvisible",boolean.class).invoke(entity,true);}catch(Exception ignored){}}
    void position(UUID uuid,org.by1337.blib.geom.Vec3d pos,float yaw,float pitch){Object entity=byUuid.get(uuid);if(entity==null)return;try{entity.getClass().getMethod("setPositionRotation",double.class,double.class,double.class,float.class,float.class).invoke(entity,pos.x,pos.y,pos.z,yaw,pitch);entity.getClass().getMethod("setInvisible",boolean.class).invoke(entity,true);}catch(Exception ignored){}}
    void hideTab(UUID uuid,org.bukkit.entity.Player viewer){Object entity=byUuid.get(uuid);if(entity==null)return;try{String nms="net.minecraft.server."+Bukkit.getServer().getClass().getPackage().getName().split("\\.")[3]+".";Class<?> entityPlayer=Class.forName(nms+"EntityPlayer"),actionType=Class.forName(nms+"PacketPlayOutPlayerInfo$EnumPlayerInfoAction");Object remove=Arrays.stream(actionType.getEnumConstants()).filter(v->v.toString().equals("REMOVE_PLAYER")).findFirst().orElseThrow();Object array=Array.newInstance(entityPlayer,1);Array.set(array,0,entity);Class<?> packetType=Class.forName(nms+"PacketPlayOutPlayerInfo");Object packet=packetType.getConstructor(actionType,array.getClass()).newInstance(remove,array);Object handle=viewer.getClass().getMethod("getHandle").invoke(viewer);Object connection=handle.getClass().getField("playerConnection").get(handle);Class<?> packetBase=Class.forName(nms+"Packet");connection.getClass().getMethod("sendPacket",packetBase).invoke(connection,packet);}catch(Exception ignored){}}

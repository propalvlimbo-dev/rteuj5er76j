...
import ac.grim.grimac.checks.impl.combat.SelfInteract;
...
public final class GrimBridge {
    ...
    private static void sync(GrimState state, Player player) {
        ...
        try {
            grim.entityID = player.getEntityId();
        } catch (Throwable ignored) {
        }
        try {
    ...
    private static final class GrimState {
        ...
        GrimState(UUID uuid, Player player) {
            ...
            add(new MultiInteractA(grim), "MultiInteract.A", config);
            add(new MultiInteractB(grim), "MultiInteract.B", config);
            add(new SelfInteract(grim), "SelfInteract.A", config);
        }
